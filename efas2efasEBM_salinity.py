...
# input files infos:
efas_input_path='/work/oda/med_dev/river_inputs_salinity/formatted_EBM/'
...
# Outfile infos 
daily_rivers=workdir+'runoff_1d_nomask_y'+str(yeartocompute)+'.nc'
runoff_var='sorunoff'
pre_runoff_var='efas_daily_runoff'
            ...
            if line[0] != '#':
               ...
               if int(efas_flag) != 0:
                  ...
                  if efas_name[0:3] != 'Po_':
                     ...
                     print ('file to read: ',efas_input_path,efas_input_filepre,efas_name,efas_input_filepost)
                  else:
                     #efas_filetoread=efas_input_path+'/'+efas_input_filepre+efas_name+'_'+efas_lat_efasidx+'_'+efas_lon_efasidx+efas_input_filepost
                     efas_filetoread=glob(efas_input_path+'/'+efas_input_filepre+efas_name[0:3]+'*_*'+efas_input_filepost)
                     print ('file to read: ',efas_filetoread)
...
with open(csv_infofile) as infile:
     for line in infile:
       if line[0] != '#':
         ...
         plt.plot(daterange,oldout[:,int(river_lat_idx),int(river_lon_idx)],'-o',label = 'EFAS river forcing (AVG='+str(np.round(np.nanmean(oldout[:,int(river_lat_idx),int(river_lon_idx)]),3))+' kg/m2/s)')
         plt.plot(daterange,newout[:,int(river_lat_idx),int(river_lon_idx)],'-o',label = 'EFAS+EBM river forcing (AVG='+str(np.round(np.nanmean(newout[:,int(river_lat_idx),int(river_lon_idx)]),3))+' kg/m2/s)')
         plt.grid ()
         ...
         #
         plt.subplot(2,1,2)
         plt.plot(daterange,diff[:,int(river_lat_idx),int(river_lon_idx)],label = '(EFAS+EBM)-EFAS (STD DEV: '+str(np.round(np.std(diff[:,int(river_lat_idx),int(river_lon_idx)]),5))+')')
         plt.grid ()
         ...
         plt.plot(daterange,newsout[:,int(river_lat_idx),int(river_lon_idx)],'-o',label = 'EFAS+EBM river forcing (AVG='+str(np.round(np.nanmean(newsout[:,int(river_lat_idx),int(river_lon_idx)]),3))+' PSU)')
         plt.grid ()
         ...
         #
         plt.subplot(2,1,2)
         plt.plot(daterange,sdiff[:,int(river_lat_idx),int(river_lon_idx)],label = '(EFAS+EBM)-Clim (STD DEV: '+str(np.round(np.std(sdiff[:,int(river_lat_idx),int(river_lon_idx)]),5))+')')
         plt.grid ()
